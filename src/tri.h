...
struct Tri {
    ...
    void compute_split(BBox& left_bb, BBox& right_bb, size_t axis, float split) const {
        left_bb = BBox::empty();
        right_bb = BBox::empty();
...
private:
    static float3 clip_edge(size_t axis, float plane, const float3& p, const float3& edge) {
