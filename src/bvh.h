...
private:
    ...
    struct Ref {
        ...
        Ref() {}
        Ref(size_t id, const BBox& bb) : id(id), bb(bb) {}
    };
    ...
    struct Node {
        ...
        size_t size() const { return ref_count; }
    };

    template <typename NodeWriter>
    int make_node(const MultiNode<Node, N>& multi_node, NodeWriter write_node) {
        int node_id = write_node(multi_node.parent / N, multi_node.parent % N, multi_node.bbox, multi_node.count, [&] (size_t i) {
    ...
    void make_leaf(const Node& node, LeafWriter write_leaf) {
        write_leaf(node.parent / N, node.parent % N, node.bbox, node.ref_count, [&] (size_t i) {
    ...
    void find_object_split(ObjectSplit& split, float3* centers, size_t axis, Ref* refs, size_t ref_count) {
        ...
        for (size_t i = ref_count - 1; i > 0; i--) {
    ...
    void apply_object_split(const ObjectSplit& split, float3* centers, Ref* refs, size_t ref_count) {
...
template <size_t N, typename CostFn>
constexpr size_t SplitBvhBuilder<N, CostFn>::spatial_bins;
template <size_t N, typename CostFn>
constexpr size_t SplitBvhBuilder<N, CostFn>::binning_passes;
